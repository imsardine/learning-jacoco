 ...
 * JaCoCo 版本的對應 https://github.com/jacoco/jacoco/wiki/ExecFileVersions[ExecFileVersions · jacoco/jacoco Wiki]
 * 怎麼看 execution data 的版本? => `org/jacoco/core/data/ExecutionDataWriter.java` 裡的 `getFileHeader()` 寫著 Returns the first bytes of a file that represents a valid execution data file. In any case every execution data file starts with the three bytes <code>0x01 0xC0 0xC0</code>.
